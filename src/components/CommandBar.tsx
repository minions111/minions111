...
export const CommandBar = ({ onCommand, commands = [] }: CommandBarProps) => {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const finalCmd = selectedIndex >= 0 ? suggestions[selectedIndex] : command;
    if (finalCmd.trim()) {
      onCommand(finalCmd.trim().toUpperCase());
      setCommand('');
      setShowSuggestions(false);
    }
  };
    ...
    <div className="bg-[#222] border-b border-[#333] p-1 flex items-center gap-2 relative z-[1000]">
      ...
      <div className="flex-1 relative">
        <form
          ...
          className="flex items-center bg-black border border-[#444] px-2 py-0.5 focus-within:border-[#ffb900]"
          role="combobox" aria-expanded={showSuggestions} aria-haspopup="listbox" aria-controls="command-suggestions"
        >
      ...
      <div className="flex gap-2 px-2 text-[10px] font-mono text-gray-400">
        ...
        <span className="hidden sm:inline">F2 GOV</span>
        <span className="hidden sm:inline">F3 CORP</span>
      </div>
    </div>
  );
};
