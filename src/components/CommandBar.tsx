...
export const CommandBar = ({ onCommand, commands = [] }: CommandBarProps) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      setSelectedIndex(p => (p <= 0 ? suggestions.length - 1 : p - 1));
          ...
          <div id="cmd-list" role="listbox" className="absolute top-full left-0 right-0 bg-black border border-[#444] shadow-2xl mt-1">
            {suggestions.map((s, i) => (
              <div
                ...
                className={cn("p-2 font-mono text-xs cursor-pointer border-b border-[#111]", selectedIndex === i ? "bg-[#ffb900] text-black" : "text-[#ffb900] hover:bg-[#ffb900]/10")}
              >{s}</div>
            ))}
          </div>
        )}
