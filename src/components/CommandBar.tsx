...
export const CommandBar = ({ onCommand, commands = [] }: CommandBarProps) => {
  ...
  return (
    <div className="bg-[#222] border-b border-[#333] p-1 flex items-center gap-2 relative z-[1000]">
      <div className="bg-[#ffb900] text-black px-2 py-0.5 font-bold text-xs" aria-hidden="true">
        HELP
      </div>
      <div className="flex-1 relative">
        <form onSubmit={handleSubmit} className="flex items-center bg-black border border-[#444] px-2 py-0.5">
          <label htmlFor="command-input" className="sr-only">Search terminal commands</label>
            ...
            {suggestions.map((s, i) => (
              <div
                ...
                className={`p-2 text-[#ffb900] font-mono text-xs cursor-pointer border-b border-[#111] transition-colors ${
                  selectedIndex === i ? 'bg-[#ffb900] text-black' : 'hover:bg-[#ffb900]/10'
                }`}
              >
                {s}
              </div>
