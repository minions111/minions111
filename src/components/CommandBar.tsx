...
export const CommandBar = ({ onCommand, commands = [] }: CommandBarProps) => {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const finalCommand = selectedIndex >= 0 ? suggestions[selectedIndex] : command.trim();

    if (finalCommand) {
      onCommand(finalCommand.toUpperCase());
      setCommand('');
      setShowSuggestions(false);
      setSelectedIndex(-1);
    }
            ...
            {suggestions.map((s, i) => (
              <div
                ...
                className={cn(
                  ...
                  i === selectedIndex ? "bg-[#ffb900] text-black" : "hover:bg-[#ffb900]/20"
                )}
              >
                {s}
              </div>
