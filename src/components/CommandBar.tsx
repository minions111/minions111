...
export const CommandBar = ({ onCommand, commands = [] }: CommandBarProps) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      onCommand(suggestions[selectedIndex]);
      setCommand('');
      setShowSuggestions(false);
  ...
  return (
    <div className="bg-[#222] border-b border-[#333] p-1 flex items-center gap-2 relative z-[1000]">
      ...
      <div className="flex-1 relative" role="combobox" aria-expanded={showSuggestions} aria-haspopup="listbox" aria-controls="cmd-list">
        <form onSubmit={handleSubmit} className="flex items-center bg-black border border-[#444] px-2 py-0.5 focus-within:border-[#ffb900]">
          <Search className="w-3 h-3 text-[#ffb900] mr-2" />
          <input
            type="text" value={command} onFocus={() => setShowSuggestions(suggestions.length > 0)} onKeyDown={handleKeyDown}
            onChange={(e) => setCommand(e.target.value.toUpperCase())}
            aria-autocomplete="list" aria-activedescendant={selectedIndex >= 0 ? `suggestion-${selectedIndex}` : undefined}
        ...
        {showSuggestions && (
          <div id="cmd-list" role="listbox" className="absolute top-full left-0 right-0 bg-black border border-[#444] shadow-2xl mt-1">
            {suggestions.map((s, i) => (
              <div
                key={i} id={`suggestion-${i}`} role="option" aria-selected={i === selectedIndex}
                onClick={() => { onCommand(s); setCommand(''); setShowSuggestions(false); }}
                className={`p-2 font-mono text-xs cursor-pointer border-b border-[#111] ${i === selectedIndex ? 'bg-[#ffb900] text-black' : 'text-[#ffb900] hover:bg-[#ffb900] hover:text-black'}`}
              >
                {s}
              </div>
            ))}
