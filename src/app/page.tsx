...
export default function Home() {
  ...
  const handleCommand = useCallback((cmd: string) => {
    ...
    setTerminals(prev => {
      const currentState = prev[activeTerminal];
      let nextView = currentState.view;
      let nextTicker = currentState.ticker;

      if (COMMAND_MAP[command]) {
        nextView = COMMAND_MAP[command];
        ...
        if (!stockSpecificViews.includes(nextView)) {
          nextView = 'MARKET';
        }
      }
      ...
      return {
        ...prev,
        [activeTerminal]: { view: nextView, ticker: nextTicker }
      };
    });
  }, [activeTerminal]);
  ...
  return (
    ...
    <main className="flex flex-col h-screen bg-black text-white font-mono overflow-hidden">
      ...
      <CommandBar onCommand={handleCommand} commands={useMemo(() => Object.keys(COMMAND_MAP), [])} />
